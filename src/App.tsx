...
import { playSequence, setBpm, startClick, stopAll, stopClick, primeAudioUnlock } from './lib/audio';
...
function App() {
  ...
  const handlePlay = async () => {
    ...
    await setBpm(bpm);
    await playSequence(sequence, { loop });
    setIsPlaying(true);
  };

  const handleMetronomeToggle = async (state: boolean) => {
    if (state) {
      await startClick();
    } else {
      stopClick();
